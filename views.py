...
def game(request):
    if request.method =='GET':
        ...
        return render(request,'bjgame2_test.html',dictionary)
        ...
        if(r.get_redis(token,'game_now') == False):
            try :
                if money < int(request.POST['bet']) or int(request.POST['bet']) <= 0:
                        raise ValueError('too large bet')
            except ValueError :
                msg = '正しくベットしてください / Place your Bet again'
                dictionary = {
                'msg' : msg ,
                'dealer_cards' : [] ,
                'dealer_point' : 0 ,
            ...
            dictionary = {
                'msg' : '選択してください / Option(Hit,Stand,Double)' ,
            ...
            if ending:
                ...
                if money <= 0 :
                    ...
                    dictionary.update(csrf(request))
                    return render(request,'gameover.html',dictionary)
